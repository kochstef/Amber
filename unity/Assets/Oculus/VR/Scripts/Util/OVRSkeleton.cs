...
public class OVRSkeleton : MonoBehaviour
{
    ...
    [SerializeField] private SkeletonType _skeletonType = SkeletonType.None;
    [SerializeField] private IOVRSkeletonDataProvider _dataProvider;
    ...
    public OVRHandPose _handPose { get; set;}
        ...
        if (_enablePhysicsCapsules)
        {
            ...
            for (int i = 0; i < skeleton.NumBoneCapsules; ++i)
            {
                ...
                var capsuleRigidBodyGO =
                    new GameObject((_bones[capsule.BoneIndex].Id).ToString() + "_CapsuleRigidBody");
    ...
    //TODO: create function to get the bone data
    public List<OVRBone> getHandPose()
    {
        var data = _dataProvider.GetSkeletonPoseData();
        if (data.IsDataValid)
        {
...
public class OVRHandPose
{
    public List<OVRBone> _bones = new List<OVRBone>();
}
